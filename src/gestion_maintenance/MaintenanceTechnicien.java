 ...
 * @author ADMIN
 */
public class MaintenanceTechnicien extends javax.swing.JFrame {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(MaintenanceTechnicien.class.getName());

    /**
     * Creates new form MaintenanceTechnicien
     */
    ...
    public MaintenanceTechnicien(int userId) throws SQLException, IOException {
        initComponents();
        this.userId = userId;
        adminId();
   ...
   private void ajouter_archive(String numImm, java.sql.Date dateMaintenance, String etatMaintenance) {
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/users", "root", "")) {
            ...
            try {
                ...
                try (PreparedStatement pstmtArchive = conn.prepareStatement(
                        "INSERT INTO archive (NumImm, date_maintenance, operation, id_user) " +
                        "VALUES (?, ?, ?, ?)")) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new MaintenanceTechnicien().setVisible(true));
    }
